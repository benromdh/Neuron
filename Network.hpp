 ...
 * potential. Also, each neuron has C_tot = 1250 imputs that came from C_ext = 1000 excitatory neurons, and C_I = 250 inhibitory ones; those are chosen randomly(thus one neuron can be selected to target 
 * the same neuron twice). Also each one receives external imput from the brain with a rate V_ext.
 ...
 * All this will enable to generate a plot representing for each step of time,all the spiking neurons.
 ***************************************************************************************************************************************************************************************************/
class Network
{
	...
	private :
		...
		double V_ext;					/*! The rate at wich the rest of the brain fires it's excitatory signal to each connected neuron */
